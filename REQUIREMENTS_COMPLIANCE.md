...
| R3 | Procurement source types include individual dealers, companies, own farms | `frontend/src/components/procurement/ProcurementFormFields.vue` | `backend/models/Procurement.js` | Compliant | `sourceType` restricted to `individual`, `company`, `own_farm`. |
...
| R9 | Only products in stock can be sold | `frontend/src/composables/useStockValidation.js` | `backend/controllers/salesController.js`, `backend/controllers/creditSalesController.js`, `backend/services/stockLockService.js` | Compliant | Server-side stock validation enforced under lock before writing sale/credit sale. |
