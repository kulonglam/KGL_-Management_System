<template>
  <div>
    ...
    <p class="text-muted">{{ user.role === 'director' ? 'All branches' : user.branch }} inventory overview</p>
          ...
          <table class="table table-hover">
            ...
            <tbody>
              <tr v-for="(item, idx) in inventory" :key="idx">
                ...
                <td class="text-end">{{ formatCurrency(item.totalTonnageKg * item.sellingPrice) }}</td>
                <td class="text-center">
                  <span v-if="item.totalTonnageKg === 0" class="badge bg-danger">Out of Stock</span>
                  <span v-else-if="item.totalTonnageKg < 500" class="badge bg-warning">Low Stock</span>
